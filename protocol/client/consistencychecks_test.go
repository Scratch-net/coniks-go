...
func TestMalformedClientMessage(t *testing.T) {
	...
	if err := cc.HandleResponse(RegistrationType, res, "", key); err != ErrMalformedMessage {
...
func TestMalformedDirectoryMessage(t *testing.T) {
	...
	if err := cc.HandleResponse(RegistrationType, res, "alice", key); err != ErrMalformedMessage {
		t.Error("Unexpected verification result")
	}
}
...
func TestVerifyRegistrationResponseWithTB(t *testing.T) {
	...
	if e1, e2 := registerAndVerify(d, cc, alice, key); e1 != ReqSuccess || e2 != nil {
		t.Error(e1)
		t.Error(e2)
	}
	...
	if e1, e2 := registerAndVerify(d, cc, alice, key); e1 != ReqNameExisted || e2 != nil {
		t.Error(e1)
		t.Error(e2)
	}
...
func TestVerifyFullfilledPromise(t *testing.T) {
	...
	if e1, e2 := registerAndVerify(d, cc, bob, key); e1 != ReqSuccess || e2 != nil {
		t.Error(e1)
		t.Error(e2)
	}
	...
	for i := 0; i < 2; i++ {
		if e1, e2 := lookupAndVerify(d, cc, alice, key); e1 != ReqSuccess || e2 != nil {
			t.Error(e1)
			t.Error(e2)
		}
	}
	...
	if e1, e2 := lookupAndVerify(d, cc, bob, key); e1 != ReqSuccess || e2 != nil {
		t.Error(e1)
		t.Error(e2)
	}
	if len(cc.TBs) != 0 {
...
func TestVerifyKeyLookupResponseWithTB(t *testing.T) {
	...
	// do lookup first
	if e1, e2 := lookupAndVerify(d, cc, alice, key); e1 != ReqNameNotFound || e2 != nil {
		t.Error(e1)
		t.Error(e2)
	}
	...
	if err := cc.HandleResponse(KeyLookupType, res, alice, nil); err != nil {
	...
	if e1, e2 := lookupAndVerify(d, cc, bob, key); e1 != ReqNameNotFound || e2 != nil {
		t.Error(e1)
		t.Error(e2)
	}
}
